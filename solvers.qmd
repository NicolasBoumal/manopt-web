...
If the step was accepted, we get that new `x`, with its `store`.
If the step was rejected, we get the same `x` as previously, with its `store`. <!-- (also the same as before, though possibly updated)-->
