...
![The `checkgradient` tool outputs a figure and text in the command line. In the figure here, we see that part of the blue curve has a slope that matches that of the dashed line: that's what we like to see. If not, then it is likely that the gradient is incorrectly implemented. Try it: change `problem.egrad` to `@(x) A*x` for example and see what happens.](images/tutorial-gradientcheck.png){width=80%}
...
As a result, Manopt automatically uses FD to approximate the Hessian if needed.
This is fine.

If we also do not specify the gradient, then Manopt approximate that with FD as well.
This is not fine: it is slow, and can hamper convergence.
This approximation is good enough that it is convenient to have as a feature for prototyping on low-dimensional manifolds, but it should not be used for anything serious.
If you use this, you may want to set `options.tolgradnorm` to a larger value and pass it to the `options` structure to the solver.
:::
