...
* `checkretraction(M, x, v)` <br>
  For manifolds `M` which have a correct exponential map `M.exp` implemented, this tool produces a slope-test plot to check the order of agreement of the retraction `M.retr` with the exponential.
