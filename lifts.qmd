...
$$
where $\calX = \varphi(\calM) \subset \calN$ is the image of the lift and $f \colon \calN \to \reals$ is the cost function.
...
If $y$ is a global minimizer for the problem upstairs, then $x$ is a global minimizer for the problem downstairs.

Moreover, depending on the properties of the lift, we may also have that if $y$ is a local minimizer then so is $x$; or that if $y$ is a (say) second-order critical point then $x$ is a (say) first-order stationary point.
General theory for this is developed in the following paper, with many examples included in Manopt:
...
| Name | $\calX$ | lift |
|:--|:---|:---|
| Box | $\{ x \in \Rn : -1 \leq x_{i} \leq 1 \, \forall i \}$ | `boxlift(n)` |
|  | $\{ X \in \Rnm : -1 \leq X_{ij} \leq 1 \, \forall i,j \}$ | `boxlift(n, m)` |
...
| Bounded-rank, psd | $S_n^p \triangleq \{ X \in \Rnn : X \succeq 0, \rank(X) \leq p \}$ | `burermonteirolift('free', n, p)` |
| With unit trace | $\{ X \in S_n^p : \trace(X) = 1 \}$ | `burermonteirolift('unittrace', n, p)` |
| With unit diagonal | $\{ X \in S_n^p : \diag(X) = \One \}$ | `burermonteirolift('unitdiag', n, p)` |
: Sets on which we can optimize with Manopt using a lift.
