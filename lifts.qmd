...
* **2 $\Rightarrow$ 2**: if $y$ is a second-order critical point upstairs, then $x$ is a first-order stationary point downstairs. This is more common.
...
: Examples of nonsmooth sets on which we can optimize with Manopt using a lift.
...
If $f$ is also convex (e.g., in the example above, if $A$ is positive semidefinite), then the problem downstairs is convex.
Therefore, first-order stationary points are global minima.
Since the lift satisfies 2 $\Righarrow$ 1, it follows that second-order critical points upstairs are global minima, even though the problem upstairs is nonconvex.
