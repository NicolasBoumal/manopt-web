...
The lift $\varphi \colon \calM \to \calN$ is the restriction of the map $\bar{\varphi} \colon \calE \to \calN$ to $\calM$, as follows:
$$\begin{align*}
...
* `('simplex', n)` sets $\calM$ to a sphere in $\Rn$ so that $\calX$ is the simplex in $\Rn$.
* `('simplex', n, m)` does the same with $\Rn$ replaced by $\Rnm$.
* `('colstochastic', n, m)` sets $\calM$ to a product of $m$ spheres in $\Rn$ so that $\calX$ is the set of column stochastic matrices of size $n \times m$.
...
The implementation is simple: it is instructive to look inside.
The key is to describe $\bar{\varphi} \colon \Rnm \to \Rnm$ and its derivatives (rather than $\varphi$ itself).
...
```matlab
lift.embedded = true;  % below describes \bar\varphi rather than \phi
lift.phi = @(Y) Y.*Y;
lift.Dphi = @(Y, V) 2*Y.*V;
