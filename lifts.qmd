...
Some lifts can also be called [with a regularizer: see below](#adding-a-regularizer).
The regularizer is activated with the `lambda` input to `manoptlift` (a real number, usually positive):
``` matlab
[upstairs, downstairs] = manoptlift(downstairs, lift, [], lambda)
```
...
: Fields in a `lift` structure. See also @tbl-liftfields-regularizer. {#tbl-liftfields}
...
Given a lift $\varphi \colon \calM \to \calN$, the lifted cost function $g = f \circ \varphi$ can be augmented with a function $\rho \colon \calM \to \reals$ and a parameter $\lambda \in \reals$ (usually positive), as follows:
$$
    g_\lambda(y) = f(\varphi(y)) + \lambda \rho(y).
$$
This $g_\lambda$ is not necessarily the lift of a function on $\calN$, because $\rho$ may not factor through $\varphi$.
Still, for $\lambda \geq 0$, it is easy to see that minimizing $g_\lambda$ is equivalent to minimizing
$$
    f_\lambda(x) = f(x) + \lambda r(x)
$$
on $\calX = \varphi(\calM)$, where $r \colon \calN \to \reals$ is defined by
$$
    r(x) = \min_{y \in \calM} \rho(y) \textrm{ subject to } \varphi(y) = x.
$$
<!--
Indeed, the value of $f(\varphi(y))$ depends on $x = \varphi(y)$, but not on the specific choice of $y$ in the fiber $\varphi^{-1}(x)$.
Whatever the minimizer of $g_\lambda$ may be, it belongs to some fiber; in that fiber, we necessarily pick an element where $\rho$ is minimal.
-->

::: {.callout-note collapse="true"}
## Click to detail why $\min g_\lambda$ and $\min f_\lambda$ are equal.
Assume $\bar y$ minimizes $g_\lambda$ and let $\bar x = \varphi(\bar y)$.
Then,
$$\begin{align*}
...
\end{align*}$$
The latter holds for all $y$.
In particular, for each $x \in \calX = \varphi(\calM)$ we can** choose $y$ such that $\varphi(y) = x$ and $\rho(y) = r(x)$,
and deduce that $f_\lambda(\bar x) \leq f(x) + \lambda r(x) = f_\lambda(x)$ for all $x$, that is, $\bar x$ minimizes $f_\lambda$ over $\calX$.
...
This becomes interesting when we consider examples: take a look at @tbl-lift-regularizers.

| lift | $\varphi \colon \calM \to \calN$ | $\rho \colon \calM \to \reals$ | $r \colon \calN \to \reals$ |
|:--|:---|:---|:---|
| `burermonteiroLRlift` | $\varphi(L, R) = LR^\top$ | $\rho(L, R) = \frac{1}{2}(\sqfrobnorm{U} + \sqfrobnorm{V})$ | $r(X) = \|X\|_{\mathrm{nuclear}}$ |
| `burermonteirolift` | $\varphi(Y) = YY^\top$ | $\rho(Y) = \sqfrobnorm{Y}$ | $r(X) = \trace(X) = \|X\|_{\mathrm{nuclear}}$ |
| `hadamarddifferencelift` | $\varphi(u, v) = u \odot u - v \odot v$ | $\rho(u, v) = \|u\|^2 + \|v\|^2$ | $r(x) = \|x\|_1$ |
: Regularizers availabe in Manopt lifts, activated with the optional input `lambda` passed to `manoptlift`. {#tbl-lift-regularizers}

To use a regularizer, the `lift` structure should provide certain fields to compute $\rho$, its gradient and (ideally) its Hessian: see @tbl-liftfields-regularizer.

| Name  | Field usage  | Functionality  |
|:-|:--|:----|
| $\rho$ | `lift.rho(y)` | Value fo the regularizer upstairs, $\rho(y)$. |
| $\grad \rho$ | `lift.gradrho(y)` | Gradient of the regularizer upstairs, $\grad \rho(y)$. If `lift.embedded = false`, this is the Riemannian gradient of $\rho$ on $\calM$. If `lift.embedded = true`, this is the gradient of $\rho$ extended to the embedding space of $\calM$, as one would do to define its `egrad`. |
| $\Hess \rho$ | `lift.hessrho(y, v)` | Hessian of the regularizer upstairs, $\Hess \rho(y)[v]$. Same comment about `lift.embedded`. |
: Fields to define a regularizer in a `lift` structure. {#tbl-liftfields-regularizer}

Such a lift can be passed to `manoptlift` with a positive `lambda` to activate the regularizer.

For example, $\varphi \colon \Rn \times \Rn \to \Rn$ defined by $\varphi(u, v) = u \odot u - v \odot v$ is a lift of $\calX = \Rn$, known as the *Hadamard difference parameterization* (`hadamarddifferencelift`).
It is known that the regularizer $\rho(u, v) = \|u\|^2 + \|v\|^2$ has the remarkable property that
$$
    r(x) = \min_{u, v : \varphi(u, v) = x} \rho(u, v) = \|x\|_1.
$$
Thus, if $f$ is smooth, then $g_\lambda$ is smooth, yet minimizing $g_\lambda$ offers a way to minimize $f_\lambda(x) = f(x) + \lambda \|x\|_1$, which is *not* smooth.
This is all the more relevant considering that the 1-norm regularizer promotes sparsity in $x$.
See this [2023 paper](https://arxiv.org/abs/2307.03571) for a discussion of that lift and associated regularizer, together with bibliographic references and a broad extension to group-sparsity regularizers and more.
<!--this [2019 paper](https://proceedings.neurips.cc/paper/2019/hash/5cf21ce30208cfffaa832c6e44bb567d-Abstract.html) for its use in the context of diagonal linear neural networks, another one [in 2020](https://proceedings.mlr.press/v125/woodworth20a.html), and  --- there's many, let's not litigate here who came up with this first. -->
Also see [this blog post](https://www.racetothebottom.xyz/posts/lift-regularizer-1norm/) for a guarantee that if $f$ is convex then $g_\lambda$ (even though it is not convex) has a benign landscape.
And see [this other blog post](https://www.racetothebottom.xyz/posts/lift-regularizer-nuclear/) for the analogous result regarding the nuclear norm regularizer.

The following example shows how to solve the [Lasso problem](https://en.wikipedia.org/wiki/Lasso_(statistics)#Least_squares) in Lagrangian form through that lift:
the aim is to minimize $f_\lambda(x) = \frac{1}{2}\|Ax - b\|^2 + \lambda\|x\|_1$ in order to recover a sparse $x \in \Rn$ from $m < n$ linear measurements.
