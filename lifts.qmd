...
## Click to show details relating $\min g_\lambda$ and $\min f_\lambda$.
...
\end{align*}$$
The latter holds for all $y$; thus, it holds in particular for all $y$ such that $r(x) = \rho(y)$ with $x = \varphi(y)$.
Therefore, $f_\lambda(\bar x) \leq f(x) + \lambda r(x) = f_\lambda(x)$ for all $x$, that is, $\bar x$ minimizes $f_\lambda$ over $\calX = \varphi(\calM)$.
:::
