...
* `problem.hess(X, Omg)` is the Riemannian Hessian at $X$ along $\dot X = X\Omega$; the input $\Omega$ represents the tangent vector $\dot X$: we only receive the skew-symmetric factor as input; the output is a tangent vector at $X$, say, $X\Zeta$, and we only output the skew-symmetric factor, so, $\Zeta$.
...
* `Omg = M.proj(X, Z)` takes a full matrix $Z \in \Rnn$ (in the embedding space) and projects it to $\T_X\SOn$. The output is in the tangent space at $X$, so it is just the skew-symmetric factor $\Omega$ that represents $\dot X = X\Omega$: the actual projection of $Z$ to $\T_X\SOn$.
